...
"""
    ...
    def _generate_dockerfile(self):
        ...
        Uses the container image specified in the pipeline's container_name.
        """
        ssh_port = self.config.get('deploy_ssh_port', 2222)

        # Get the container image name from the pipeline
        if hasattr(self.pipeline, 'container_name') and self.pipeline.container_name:
            base_image = self.pipeline.container_name
        else:
            # Fallback to default if pipeline doesn't have container_name
